<?php
  ...
  class Optimizer
  {
    ...
    public static function run()
    {
      ...
      $booster_path    = APLibPath."boost-".\APLib\Config::get('Optimize name').".php";
    	$booster_html    = APLibHTML."boost-".\APLib\Config::get('Optimize name').".php";
      $files           = array();
      ...
      \APLib\Response\Header::reset();
        ...
        $files[md5($file)] = $file;
        \APLib\Response\Header::add("<link rel='stylesheet' href=\"".$booster_html."?file=".md5($file)."\" async />");
			}
        ...
        $files[md5($file)] = $file;
        \APLib\Response\Header::add("<script type='text/javascript' src=\"".$booster_html."?file=".md5($file)."\"></script>");
			}
      ...
      $booster_content  = "<?php\r\n";
      $booster_content .= "if(!isset(\$_GET['file'])) die();\r\n";
      ...
      $booster_content .= "   die();\r\n";
      $booster_content .= " }\r\n";
      $booster_content .= " \$ext   = explode('.', '../'.\$file)[sizeof(explode('.', '../'.\$file))-1];\r\n";
      $booster_content .= " if(\$ext == 'js'){\r\n";
			$booster_content .= "		header(\"Content-Type: application/javascript\");\r\n";
			$booster_content .= "	}\r\n";
      $booster_content .= " if(\$ext == 'css'){\r\n";
			$booster_content .= "		header(\"Content-Type: text/css\");\r\n";
			$booster_content .= "	}\r\n";
      $booster_content .= "	die(\$content);\r\n";
			$booster_content .= "}\r\n";
			$booster_content .= "?>";
			if(file_exists($booster_path))
			{
				if(file_get_contents($booster_path) == $booster_content) goto ok;
			}
			file_put_contents($booster_path, $booster_content);
			ok:
    }
  }
?>
